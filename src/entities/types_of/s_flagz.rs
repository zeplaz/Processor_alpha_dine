...
use super::p_enumz::ConcreateType;
...
pub enum ApartmentsType {
    ...
    ThreeStoryBlock { units_available: Vec<ApartmentUnitType> },
    FiveStoryBlock { units_available: Vec<ApartmentUnitType> },
}
...
impl ApartmentsType {
    fn total_rooms(&self) -> f32 {
        match *self {
            ...
            ApartmentsType::ThreeStoryBlock { ref units_available } => units_available

                .iter()
                .map(|x| x.total_rooms())
                .sum::<f32>(),

            ApartmentsType::FiveStoryBlock { ref units_available } => units_available   
                .iter()
                .map(|x| x.total_rooms()) 
                .sum::<f32>(),
